    ...
    <div className="rounded-div my-12 py-8">
      ...
      <div className="grid md:grid-cols-2 gap-8">
        ...
        <div>
          ...
          <div className="flex justify-between py-4">
            <div>
              <p className="text-gary-500 text-sm">Market Rank</p>
              {coin?.market_cap_rank}
            </div>
            <div>
              <p className="text-gary-500 text-sm">Hashing Algoritm</p>
              {coin?.hashing_algoritm ? <p>{coin?.hashing_algoritm}</p> : null}
            </div>
            <div>
              <p className="text-gary-500 text-sm">Trast Score</p>
              {coin.tickers ? <p>{coin?.liquidity_score.toFixed(2)}</p> : null}
            </div>
          </div>
          <div className="flex justify-between py-4">
            <div>
              <p className="text-gary-500 text-sm">Price Change (24)</p>
              {coin.market_data ? (
                <p>{coin.market_data?.price_change_percentage_24h.toFixed(2)}%</p>
              ) : null}
            </div>
            <div>
              <p className="text-gary-500 text-sm">Price Change (7d)</p>
              {coin.market_data ? (
                <p>{coin.market_data?.price_change_percentage_7d.toFixed(2)}%</p>
              ) : null}
            </div>
            <div>
              <p className="text-gary-500 text-sm">Price Change (14d)</p>
              {coin.market_data ? (
          ...
          <div className="flex justify-between py-4">
            <div>
              <p className="text-gary-500 text-sm">Price Change (30d)</p>
              {coin.market_data ? (
                <p>{coin.market_data?.price_change_percentage_30d.toFixed(2)}%</p>
              ) : null}
            </div>
            <div>
              <p className="text-gary-500 text-sm">Price Change (60d)</p>
              {coin.market_data ? (
                <p>{coin.market_data?.price_change_percentage_60d.toFixed(2)}%</p>
              ) : null}
            </div>
            <div>
              <p className="text-gary-500 text-sm">Price Change (1y)</p>
              {coin.market_data ? (
